...
use crypto_bigint::{U2048, U256, U4096};
use num_bigint::BigUint;
use sp1_zkvm::syscalls::syscall_u256x2048_mul;
use bytemuck::cast_ref;

pub fn main() {
    let iter: u32 = sp1_zkvm::io::read();
    let a: Vec<u8> = sp1_zkvm::io::read();
    let b: Vec<u8> = sp1_zkvm::io::read();

    let a_u2048 = from_biguint_to_u2048(&BigUint::from_bytes_le(&a));
    let b_u2048 = from_biguint_to_u2048(&BigUint::from_bytes_le(&b));
    for _ in 0..iter {
        mul_u2048(a_u2048, b_u2048);
    }

    sp1_zkvm::io::commit(&iter);
}
